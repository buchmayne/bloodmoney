...
# From the official ufc site extract all past events
...
event_ids_df = event_ids_df.reset_index(drop=True)

example_card_suffix = event_ids_df.loc[0, 'event_url']
example_url_card = url_card_format.format(example_card_suffix)

raw_html = get(example_url_card, headers=headers)
html = BeautifulSoup(raw_html.content, 'html.parser')
...
for line in html.select('div'):
    # Checking to see if it contains the fight card number
    try:
        if line['id'] == 'c-listing-ticker':
            fight_card = (line['data-fmid'])
    except KeyError:
        pass
...
data[example_card_suffix] = dict()
data[example_card_suffix]['fight_ids'] = fight_ids
data[example_card_suffix]['fighter_1'] = fighter_1
data[example_card_suffix]['fighter_2'] = fighter_2
data[example_card_suffix]['odds'] = odds

example_event_data = pd.DataFrame(data[example_card_suffix])
example_event_data['event_name'] = example_card_suffix
